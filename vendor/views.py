...
def add_food(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            form.save()
            messages.success(request, 'The food item has been successfully added!')
...
def edit_food(request, pk=None):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            food.slug = slugify(foodtitle)
            form.save()
            messages.success(request, 'The food item has been successfully updated!')
            return redirect('fooditems_by_category', food.category.id)
        else:
            print(form.errors)
    else:
...
def delete_food(request, pk=None):
    food = get_object_or_404(FoodItem, pk=pk)
    food.delete()
    messages.success(request, 'The food item has been successfully deleted!')
    return redirect('fooditems_by_category', food.category.id)
...
def add_opening_hours(request):
    # handle the data and save them insite the database
